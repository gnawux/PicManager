...
picmanager activities import <path>                   # 导入 FIT/GPX 文件或目录
picmanager activities import <path> --dry-run         # 仅统计，不写库
picmanager activities sync-usb                        # 从 USB Garmin 设备导入
picmanager activities update-titles [--dry-run]       # 为 title 为空的运动补全自动标题
picmanager activities fix-metadata [--dry-run]        # 从已保存 FIT 重新读取设备/传感器信息
