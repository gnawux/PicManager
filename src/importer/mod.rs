...
async fn import_one(
    ...
    let result = sqlx::query(
        "INSERT OR IGNORE INTO photos (path, sha256, phash, format, taken_at, timezone_offset, gps_lat, gps_lon, camera, import_status, exif_orientation)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'imported', ?)",
    )
