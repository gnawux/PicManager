...
> **注意：** 签名步骤是必须的。未签名时，macOS TCC 无法识别该二进制，
> `requestAuthorization` 会直接返回 `denied`，不弹出任何授权对话框。
> 每次 `swift build -c release` 之后都需要重新签名。
>
> 如果签名后仍被拒绝（例如之前已授权后又撤销），可重置 TCC 后重新运行：
> ```bash
