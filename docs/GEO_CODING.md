...
地名统一任务以四位小数规范化后的**唯一坐标**为进度单位，不以照片数量为单位。
坐标按经纬度排序，相同坐标只解析一次，成功写入的 policy revision 同时作为重启
检查点；同一坐标的所有照片在一个事务中更新地点相册关系。连续三次 provider
连接或 HTTP 失败会让任务快速进入可重试状态，避免离线时逐项等待 10 秒。任务每
两秒至多写一次变化后的进度，并在每个有超时上限的远程请求之间检查取消状态。

Mac App 启动服务时会读取 macOS 已启用的手动 HTTP、HTTPS 和 SOCKS 代理并传给
Rust 服务。命令行模式仍可使用标准 `HTTP_PROXY`、`HTTPS_PROXY`、`ALL_PROXY` 和
`NO_PROXY` 环境变量。更改系统代理后需要重启 PicManager；PAC 自动配置目前不会
被展开为固定代理地址。
