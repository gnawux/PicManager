...
state：计算 SHA-256
    │
    ├─── 已存在 → 标记 duplicate / already_imported，跳过
    │
    ▼
metadata：提取 EXIF（时间、GPS、相机）
    │
    ▼
dedup/hash：计算感知哈希（dHash），写入 photos.phash
    │
    ▼
storage：写入 photos 表，import_status = 'imported'
    │
    ▼
